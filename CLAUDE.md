...
Single module: `app.py` covers discovery (`scan`/`detect`), action endpoints
...
`data/attachments/`) is bracketed-pasted into the chat's input but never
...
(`appearance:none`), and marking done raises an `undoToast`, so accidental taps
get one-tap recovery without a confirmation step. Every done-flip goes through
...
request, so JS/CSS changes deploy with just a browser refresh and no restart). New
...
mounts without p5: the XP hill + clouds, a one-off dither tile, the spectrum
...
⋯ menu and project picker, are intentionally excluded; they dismiss on any tap/Esc).
...
- **NEVER `systemctl --user restart hub.service` without checking your own
  ...
  hub's drawer used to run *inside* hub.service, so restarting killed every
  ...
  re-read per request and need no restart; `app.py` links its JS/CSS via
...
- **Persistent sessions**: `tools/hub_ptyd.py` holds each chat's pty and
  ...
  reporting xterm turns the wheel into arrow keys (Claude navigates rather than
  scrolls; it scrolls on PgUp/PgDn), so on **desktop** a capture-phase `wheel`
  ...
  `tests/test_persist.py` (its own uvicorn on :7799; it never touches the
  live hub).
...
- Static-site projects (`index.html`) are served **dynamically** by
  `serve_site` under `/sites/<name>/` (path-traversal guarded), so a freshly
  cloned/created site works immediately without a restart.
...
Anything committed here is visible to anyone, effectively forever: forks,
mirrors, and archive crawlers survive later privatization. Before every commit:

- No secrets: no tokens, API keys, passwords, pairing codes, or private URLs,
  not even inside screenshots, QR codes, or example output.
- No personal data: no addresses, phone numbers, order numbers, or other
  people's names and handles without their consent.
- Never name or identify the author's employer or its partner companies, in
  text, photos, Wi-Fi network names, banners, or file metadata.
- Audit every new image before committing: check backgrounds, screens, labels,
  and reflections for any of the above.
- Public-facing prose (README, docs, site pages) goes through the humanizer
  skill: no em dashes, no AI-writing tells.
- New vendored assets keep their license and attribution files.
