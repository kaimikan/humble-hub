...
between messages), but that hands every drag to the program and breaks plain
...
you: a settled selection lands on the clipboard automatically, with no key
combo or right-click needed.
...
"jiggle": resize the pty one column down and back up to force a SIGWINCH
...
field keeps it in its own category while still wiring the controls:
...
Further out: voice-orchestrated sessions, where "let's work on X" spoken to
the root chat spawns a project chat with its own status pill.
